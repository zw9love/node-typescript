...
export default class BeeneedleProcessSubject {
    ...
    upDateData(json: any, successFn?: Function, errorFn?: Function): void {
        let sql = `UPDATE ${this.tableName} SET name = ?, path = ?, hash = ?,sens_value = ?,reli_value = ? where ids = ?`;
        let arr = [json.name, json.path, json.hash, json.sens_value, json.reli_value, json.ids]
    ...
    */
    ...
    addData(json: any, successFn?: Function, errorFn?: Function): void {
        let sql = `INSERT INTO ${this.tableName} (ids, name, path, sens_value, reli_value) VALUES ( ?, ?, ?, ?, ?) `
        let ids = getRandomString()
        let arr = [ids, json.name, json.path, 150, 150]
