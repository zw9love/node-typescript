...
export default class BeeneedleProtect {
    public dao = new Dao()
    private tableName: string = 'beeneedle_protected'
    constructor() { }
    /**
    ...
    upDateData(json: any, successFn?: Function, errorFn?: Function): void {
        let sql = `UPDATE ${this.tableName} SET name = ?, full_path = ? where ids = ?`;
        let arr = [json.name, json.full_path, json.ids]
    ...
    */
    ...
    addData(json: any, successFn?: Function, errorFn?: Function): void {
        let sql = `INSERT INTO ${this.tableName} (ids, name, full_path, host_ids, type) VALUES ( ?, ?, ?, ?, ?) `
        let ids = getRandomString()
        let arr = [ids, json.name, json.full_path, json.host_ids, json.type]
        // 检查进程名和路径是否重复
        let checkSql = `SELECT count(*) as sum FROM ${this.tableName} where name = ? and full_path = ?`
        this.dao.connectDatabase(checkSql, [json.name, json.full_path], res => {
            if (res[0].sum > 0) {
