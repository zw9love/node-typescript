...
import { response, request, loginData, postData } from '../interface/index'
...
export default class BeeneedlePelf {
    ...
    private tableName: string = 'beeeye_pelf'
    constructor() { }
    /**
     * @description 查询用户,排序，模糊查找
     * @param data  sql语句参数
    ...
    getData(postData: postData, successFn?: Function, errorFn?: Function): void {
        ...
        let pelfstatus = row.pelfstatus
        let select = `SELECT * FROM ${this.tableName} `
        let where = ` where pelfstatus = ? `
        ...
        let tsData = [
            { sql: count + where, dataArr: [pelfstatus, hostIds] },
            { sql: select + where + search + sortSql + limit, dataArr: [pelfstatus, hostIds] },
        ]
    ...
    upDateData(json: any, successFn?: Function, errorFn?: Function): void {
        let sql = `UPDATE ${this.tableName} SET pelfstatus = ? where ids = ?`;
        let arr = [json.pelfstatus, json.ids]
